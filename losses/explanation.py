...
class ExplanationLoss(nn.Module):
    ...
    def forward(
        ...
        # Time-average each sample, then compare sample centroids.
        # This is much more stable than comparing every frame individually.
        ...
        l_div_tensor = F.relu(
            sim_matrix.masked_fill(eye, 0.0).sum() / max(n_pairs, 1) - 0.5
        )
        loss = loss + self.diversity_weight * l_div_tensor

        # ── Class-conditional separation ───────────────────────────────────────
        # Penalise when real and fake attention centroids are too similar.
        ...
        if labels is not None and B >= 2:
            ...
            if real_mask.any() and fake_mask.any():
                ...
                # Hinge: allow up to 0.2 cosine similarity, penalise above
                l_class_sep = F.relu(sim - 0.2)
                loss = loss + self.diversity_weight * l_class_sep
