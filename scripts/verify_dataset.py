...
def main():
    ...
    try:
        ...
        if n_real_in_batch == 0 or n_fake_in_batch == 0:
            failures.append(
                f"Batch is unbalanced: real={n_real_in_batch}, fake={n_fake_in_batch}."
            )
    ...
    if batch is not None:
        try:
            ...
            if mt.min() == mt.max():
                failures.append(
                    "M_t is constant (all same value). Model is not producing spatial attention."
                )
        except Exception as exc:
        ...
        failures.append("Forward pass skipped — batch was not loaded.")
